...
import com.lexinfintech.android.arouter.exception.NoRouteFoundException;
import com.lexinfintech.android.arouter.facade.Postcard;
import com.lexinfintech.android.arouter.facade.enums.TypeKind;
import com.lexinfintech.android.arouter.facade.model.RouteMeta;
import com.lexinfintech.android.arouter.facade.template.IInterceptorGroup;
import com.lexinfintech.android.arouter.facade.template.IProvider;
import com.lexinfintech.android.arouter.facade.template.IProviderGroup;
import com.lexinfintech.android.arouter.facade.template.IRouteGroup;
import com.lexinfintech.android.arouter.facade.template.IRouteRoot;
import com.lexinfintech.android.arouter.launcher.ARouter;
import com.lexinfintech.android.arouter.utils.ClassUtils;
import com.lexinfintech.android.arouter.utils.Consts;
import com.lexinfintech.android.arouter.utils.MapUtils;
import com.lexinfintech.android.arouter.utils.PackageUtils;
import com.lexinfintech.android.arouter.utils.TextUtils;
...
import static com.lexinfintech.android.arouter.launcher.ARouter.logger;
...
public class LogisticsCenter {
        ...
        if (!TextUtils.isEmpty(className)) {
            try {
                ...
                if (obj instanceof IRouteRoot) {
                    ...
                    logger.info(Consts.TAG, "register failed, class name: " + className
                            + " should implements one of IRouteRoot/IProviderGroup/IInterceptorGroup.");
                }
            } catch (Exception e) {
                logger.error(Consts.TAG,"register class error:" + className);
            }
        }
    }
     ...
     * @param routeRoot IRouteRoot implementation class in the package: com.lexinfintech.android.arouter.core.routers
     * @author billy.qi <a href="mailto:qiyilike@163.com">Contact me.</a>
     * @since 2017-12-06
     */
     ...
     * @param interceptorGroup IInterceptorGroup implementation class in the package: com.lexinfintech.android.arouter.core.routers
     * @author billy.qi <a href="mailto:qiyilike@163.com">Contact me.</a>
     * @since 2017-12-06
     */
     ...
     * @param providerGroup IProviderGroup implementation class in the package: com.lexinfintech.android.arouter.core.routers
     * @author billy.qi <a href="mailto:qiyilike@163.com">Contact me.</a>
     * @since 2017-12-06
     */
    ...
    public synchronized static void init(Context context, ThreadPoolExecutor tpe) throws HandlerException {
        ...
        try {
            ...
            loadRouterMap();
            if (registerByPlugin) {
                logger.info(Consts.TAG, "Load router map by arouter-auto-register plugin.");
            } else {
                Set<String> routerMap;

                // It will rebuild router map every times when debuggable.
                if (ARouter.debuggable() || PackageUtils.isNewVersion(context)) {
                    logger.info(Consts.TAG, "Run with debug mode or new install, rebuild router map.");
                    // These class was generated by arouter-compiler.
                    routerMap = ClassUtils.getFileNameByPackageName(mContext, Consts.ROUTE_ROOT_PAKCAGE);
                    if (!routerMap.isEmpty()) {
                        context.getSharedPreferences(Consts.AROUTER_SP_CACHE_KEY, Context.MODE_PRIVATE).edit().putStringSet(Consts.AROUTER_SP_KEY_MAP, routerMap).apply();
                    }

                    PackageUtils.updateVersion(context);    // Save new version name when router map update finishes.
                } else {
                    logger.info(Consts.TAG, "Load router map from cache.");
                    routerMap = new HashSet<>(context.getSharedPreferences(Consts.AROUTER_SP_CACHE_KEY, Context.MODE_PRIVATE).getStringSet(Consts.AROUTER_SP_KEY_MAP, new HashSet<String>()));
                }

                logger.info(Consts.TAG, "Find router map finished, map size = " + routerMap.size() + ", cost " + (System.currentTimeMillis() - startInit) + " ms.");
            ...
            logger.info(Consts.TAG, "Load root element finished, cost " + (System.currentTimeMillis() - startInit) + " ms.");

            if (Warehouse.groupsIndex.size() == 0) {
                logger.error(Consts.TAG, "No mapping files were found, check your configuration please!");
            }

            if (ARouter.debuggable()) {
                logger.debug(Consts.TAG, String.format(Locale.getDefault(), "LogisticsCenter has already been loaded, GroupIndex[%d], InterceptorIndex[%d], ProviderIndex[%d]", Warehouse.groupsIndex.size(), Warehouse.interceptorsIndex.size(), Warehouse.providersIndex.size()));
            }
        } catch (Exception e) {
    ...
    public synchronized static void completion(Postcard postcard) {
        ...
        if (null == routeMeta) {    // Maybe its does't exist, or didn't load.
            ...
            if (null == groupMeta) {
                ...
                try {
                    if (ARouter.debuggable()) {
                        logger.debug(Consts.TAG, String.format(Locale.getDefault(), "The group [%s] starts loading, trigger by [%s]", postcard.getGroup(), postcard.getPath()));
                    }
                    ...
                    if (ARouter.debuggable()) {
                        logger.debug(Consts.TAG, String.format(Locale.getDefault(), "The group [%s] has already been loaded, trigger by [%s]", postcard.getGroup(), postcard.getPath()));
                    }
                } catch (Exception e) {
